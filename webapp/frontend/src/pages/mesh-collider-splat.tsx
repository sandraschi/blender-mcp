import { AlertCircle, Box, Download, Layers, Package, Plus, Search } from "lucide-react";
    ...
    try {
      ...
      const msg =
        res.success && res.data != null
          ? typeof res.data === "object" && res.data && "output" in res.data
            ? (res.data as { output?: string }).output
            : String(res.data)
    ...
    <div className="p-6 max-w-4xl space-y-6">
      ...
      {backendOk === false && (
        <div className="flex items-center gap-2 p-3 rounded-lg bg-destructive/10 text-destructive">
          ...
          <button type="button" onClick={checkBackend} className="ml-auto text-sm underline">
            Retry
          </button>
        </div>
      )}
              ...
              <ul className="mt-3 space-y-1 text-sm">
                {addonsInstalled.map((a) => (
                  <li key={a.name} className="flex items-center gap-2">
                    <span>{a.name}</span>
                    <span className={a.enabled ? "text-green-600" : "text-muted-foreground"}>
