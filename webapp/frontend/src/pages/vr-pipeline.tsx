...
import { Glasses, CheckCircle, AlertTriangle, ArrowRight } from 'lucide-react'
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
                {[
                    ...
                    <button
                        ...
                        className={`p-4 rounded-lg border text-left transition-all ${platform === p.id
                ...
                <div className="flex justify-between relative z-10">
                    {steps.map((step) => (
                        <div key={step.id} className="flex flex-col items-center">
                            <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-colors ${step.id < activeStep
                            ...
                            <span className={`mt-2 text-xs font-medium ${step.id === activeStep ? 'text-primary' : 'text-muted-foreground'}`}>
                                {step.name}
                            </span>
                        </div>
                    ))}
                ...
                <div className="space-y-4 mb-6">
                    ...
                    <div className="p-4 bg-yellow-500/5 border border-yellow-500/20 rounded-lg flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <button className="mt-2 text-xs bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-500 px-3 py-1.5 rounded transition-colors">
                                Auto-Fix Weights
                            </button>
                        </div>
                    </div>
