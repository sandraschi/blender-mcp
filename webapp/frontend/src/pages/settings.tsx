...
import { getConfig, updateConfig, listLocalModels } from '../api/mcp'
...
export default function Settings() {
    ...
    const fetchModels = async () => {
        ...
        try {
            ...
            const ollamaModels = (ollama ?? []).map((name: string) => ({ id: name, name, loaded: false }))
            const lmStudioModels = (lm_studio ?? []).map((id: string) => ({ id, name: id, loaded: true }))
                        ...
                        <div className="p-4 pt-0 space-y-6">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <button
                                    ...
                                    className={`p-3 rounded-lg border text-left transition-colors ${llmConfig.provider === 'ollama'
                                        ? 'border-primary bg-primary/10'
                                        : 'border-border hover:border-primary/50'
                                        }`}
                                >
                                    <div className="flex items-center gap-2 mb-1">
                                        <Cpu className={`w-4 h-4 ${llmConfig.provider === 'ollama' ? 'text-primary' : 'text-muted-foreground'}`} />
                                ...
                                <button
                                    ...
                                    className={`p-3 rounded-lg border text-left transition-colors ${llmConfig.provider === 'lmstudio'
                                        ? 'border-primary bg-primary/10'
                                        : 'border-border hover:border-primary/50'
                                        }`}
                                >
                                    <div className="flex items-center gap-2 mb-1">
                                        <Server className={`w-4 h-4 ${llmConfig.provider === 'lmstudio' ? 'text-primary' : 'text-muted-foreground'}`} />
                                ...
                                <button
                                    ...
                                    className={`p-3 rounded-lg border text-left transition-colors ${llmConfig.provider === 'cloud'
                                        ? 'border-primary bg-primary/10'
                                        : 'border-border hover:border-primary/50'
                                        }`}
                                >
                                    <div className="flex items-center gap-2 mb-1">
                                        <Cloud className={`w-4 h-4 ${llmConfig.provider === 'cloud' ? 'text-primary' : 'text-muted-foreground'}`} />
                                        ...
                                        <div className="border border-border rounded-md divide-y divide-border max-h-48 overflow-y-auto">
                                            {loadingModels ? (
                                                ...
                                                availableModels.map(model => (
                                                    <div
                                                        key={model.id}
                                                        className={`p-3 flex items-center justify-between hover:bg-accent/50 cursor-pointer ${llmConfig.selected_model === model.id ? 'bg-primary/10' : ''
                                                            }`}
                                                    >
                                                        <div>
                            ...
                            {llmConfig.provider === 'lmstudio' && (
                                <div className="space-y-4">
                                    <div>
                                        <label className="text-sm font-medium mb-1 block">LM Studio URL</label>
                                        <input
                                            id="lmstudio-url"
                                            type="text"
                                            ...
                                            className="w-full bg-background border border-input rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-ring"
                                        />
                                        <p className="text-xs text-muted-foreground mt-1">Default: http://localhost:1234</p>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            placeholder="sk-..."
                                        />
                                        <p className="text-xs text-muted-foreground mt-1">Your API key is stored locally and never shared.</p>
                                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                value={serverHost}
                                onChange={(e) => { setServerHost(e.target.value); setHasChanges(true); }}
                                className="w-full bg-background border border-input rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-ring"
                            />
                        </div>
                        <div>
                            <label className="text-sm font-medium mb-1 block">Server Port</label>
                            <input
                                type="number"
                                id="server-port"
                                title="Server Port Number"
                                value={serverPort}
                                onChange={(e) => { setServerPort(parseInt(e.target.value)); setHasChanges(true); }}
                                className="w-full bg-background border border-input rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-ring"
                            />
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">Automatically sync changes to Blender</p>
                            </div>
                            <button
                                onClick={() => { setAutoSync(!autoSync); setHasChanges(true); }}
                            ...
                            >
                                <div className={`w-5 h-5 rounded-full bg-background transition-transform ${autoSync ? 'translate-x-6' : 'translate-x-0.5'}`} />
                            </button>
                        </div>
                        <div className="flex items-center justify-between">
                            <div>
                            ...
                            >
                                <div className={`w-5 h-5 rounded-full bg-background transition-transform ${notifications ? 'translate-x-6' : 'translate-x-0.5'}`} />
                            </button>
                        </div>
                    </div>
                </div>
