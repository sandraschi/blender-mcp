 ...
 */
export async function getSceneData(): Promise<
  MCPResponse<{
    collections: Array<{
      name: string;
      objects: Array<{
 ...
 */
export async function executeAICommand(prompt: string): Promise<
  MCPResponse<{
    result: string;
 ...
 */
export async function executeScript(code: string): Promise<
  MCPResponse<{
    output: string;
    success: boolean;
  }>
...
> {
  const res = await callTool<{
    success?: boolean;
    result?: { ollama?: string[]; lm_studio?: string[]; errors?: string[] };
    summary?: string;
  }>("list_local_models", {});
 ...
 */
export async function downloadAndImport(url: string, importIntoScene = true): Promise<MCPResponse> {
  return callTool("blender_download", {
    operation: "download",
    url,
    import_into_scene: importIntoScene,
  });
}
