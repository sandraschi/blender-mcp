...
export function FleetCard({ member, currentAppId }: FleetCardProps) {
    ...
    const checkStatus = async () => {
        try {
            ...
            const resp = await fetch(`http://localhost:${member.port}/api/v1/health`, { signal: controller.signal });
            clearTimeout(id);
    ...
    return (
        <motion.div
            ...
            className={`group relative overflow-hidden rounded-xl border p-5 transition-all duration-300 ${isCurrent
        ...
        >
            <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                    <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${isCurrent ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground group-hover:text-foreground"
                        }`}>
                ...
                <div className="flex items-center gap-1.5">
                    <div className={`h-2 w-2 rounded-full ${status === 'online' ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" :
                        status === 'offline' ? "bg-muted" : "bg-primary animate-pulse"
                        }`} />
            ...
            <div className="mt-6 flex gap-2">
                {status === 'online' ? (
                    ...
                    <button
                        ...
                        className={`flex flex-1 items-center justify-center gap-2 rounded-lg py-2 text-xs font-medium transition-all active:scale-95 ${isLaunching
