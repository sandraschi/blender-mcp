...
async def mesh_inset(
    object_name: str, thickness: float = 0.1, depth: float = 0.0, prefer_session: bool = True
) -> dict[str, Any]:
    body = (
        _object_setup(object_name)
        + f"""
...
async def mesh_merge_vertices(
    object_name: str, merge_distance: float = 0.001, prefer_session: bool = True
) -> dict[str, Any]:
    body = (
        _object_setup(object_name)
        + f"""
