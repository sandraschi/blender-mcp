    ...
    def decorator(func: F) -> F:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                execution_time = time.time() - start_time
                logger.error(
                    f"Blender operation failed: {operation_name} [{operation_id}] ({execution_time:.2f}s) - {e}"
                )
                raise
                ...
                raise BlenderMCPError(f"Unexpected error in {operation_name}: {e!s}", "UNEXPECTED_ERROR") from e
